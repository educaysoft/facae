<?php
include("head0.php");
?>




<?php
include("head.php");
?>
<div style="  display:flex;flex-direction: column; background-color: white;">
<div style="width: 60%; margin:0px auto;">


<main id="agenda" class="cd-main-content" style="margin-top:0%">
<p style="font-size: 30px; font-weight: bold; text-align: center; padding: 20px 0 40px 0; "> MEMORIA</p>

<center>
<div style="padding:20px;">
Evidencias del trabajo realizado durante todo el proceso de difusión, organización y ejecución del evento.
</div >
</center>




<!------

<div style="margin-bottom: 10px; display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/cordinadorevento.jpg" style="width:100%; ">

  </div>
  
  
	</div>  
--->
<div style="padding:20px;">
Primer tríptico promosional.
</div >

<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/PrimeraPublicacionFacebook.jpg" style="width:100%; ">
  </div>
</div>



  


<div style="padding:20px;">
Promosión Eje temático #4.
</div >

<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/PromosionEjeTematico4.jpeg" style="width:100%; ">
  </div>
  
  
	</div>


  



<div style="padding:20px;">
Mesa de trabajo para afinar detalles en la plataforma Web del Congreso.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/MesaDeTrabajoPlataformaWeb.jpeg" style="width:100%; ">
  </div>
  
  
	</div>  




<div style="pading:20px;">
Promoción Ejes Temáticos 1 y 2.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/PromosionEjeTematico-1-2.jpeg" style="width:100%; ">
  </div>
  
  
	</div>  




<div style="padding:20px;">
Promoción Ejes Temáticos 3 y 4.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/PromosionEjeTematico-3-4.jpeg" style="width:100%; ">
  </div>
	</div>  


<div style="padding:20px;">
Promoción Ejes Temáticos 5,6 y 7.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/PromosionEjeTematico-5-6-7.jpeg" style="width:100%; ">
  </div>
	</div>  












<div style="padding:20px;">
Ponencia magistral .
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/flyer8.jpg" style="width:100%; ">
  </div>
	</div>  




<div style="padding:20px;">
Ponencia magistral .
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/flyer9.jpg" style="width:100%; ">
  </div>
	</div>  



<div style="padding:20px;">
Ponencia magistral .
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/flyer10.jpg" style="width:100%; ">
  </div>
	</div>  

<div style="padding:20px;">
Ponencia magistral.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/flyer11.jpg" style="width:100%; ">
  </div>
	</div>  



<div style="padding:20px;">
Ponencia magistral.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/flyer12.jpg" style="width:100%; ">
  </div>
	</div>  


<div style="padding:20px;">
Ponencia magistral.
</div>

<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/flyer13.jpg" style="width:100%; ">
  </div>
	</div>  



<div style="padding:20px;">
Ponencia magistral.
</div>


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/flyer14.jpg" style="width:100%; ">
  </div>
	</div>  





<div style="padding:20px;">
Ponencia magistral.
</div>


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/flyer15.jpg" style="width:100%; ">
  </div>
	</div>  







</main>
</div>









<?php

include("footer.php");
?>
</div>
