<?php
include("head0.php");
?>




<?php
include("head.php");
?>
<div style="  display:flex;flex-direction: column; background-color: white;">
<div style="width: 60%; margin:0px auto;">


<main id="agenda" class="cd-main-content" style="margin-top:0%">
<p style="font-size: 30px; font-weight: bold; text-align: center; padding: 20px 0 40px 0; "> III COLOQUIO ESTUDIANTIL "JOVENES INVESTIGADORES"</p>


<center>
      <img src="images/coloquio/qrcoloquio.png" style="width:100%; ">
</center>
<center>
<div style="padding:20px;font-size:20px">
Eje : Emprendimiento sostenible e inclusivo:  Como fomentar el espíritu emprendedor entre los estudiantes y promover modelos  de negocios que integren aspectos administrativos, sociales y ambientales.
</div >
</center>




<!------

<div style="margin-bottom: 10px; display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/Memoria/cordinadorevento.jpg" style="width:100%; ">

  </div>
  
  
	</div>  
--->
<div style="padding:20px;">
09:00 - 09:15
</div >

<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/coloquio/coloquiocharla.jpg" style="width:100%; ">
  </div>
</div>



  


<div style="padding:20px;">
09:20 - 09:40
</div >

<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/coloquio/coloquioponente1.jpg" style="width:100%; ">
  </div>
  
  
	</div>


  



<div style="padding:20px;">
9:45 - 10:05.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/coloquio/coloquioponente2.jpg" style="width:100%; ">
  </div>
  
  
	</div>  




<div style="padding:20px;">
10:10 - 10:30.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/coloquio/coloquioponente3.jpg" style="width:100%; ">
  </div>
  
  
	</div>  




<div style="padding:20px;">
10:35 - 10:55.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/coloquio/coloquioponente4.jpg" style="width:100%; ">
  </div>
	</div>  


<div style="padding:20px;">
11:00 - 11:20.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/coloquio/coloquioponente5.jpg" style="width:100%; ">
  </div>
	</div>  












<div style="padding:20px;">
11:25 - 11:45.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/coloquio/coloquioponente6.jpg" style="width:100%; ">
  </div>
	</div>  




<div style="padding:20px;">
11:50 - 12:10.
</div >


<div style=" display: flex; flex-direction:row; justify-content: center; width:100%;" >
  <div  class="w3-card-4"  style="border:2px solid green; width: 100%; display:flex; justify-content: center; align-items: center;">
      <img src="images/coloquio/coloquioponente7.jpg" style="width:100%; ">
  </div>
	</div>  







</main>
</div>









<?php

include("footer.php");
?>
</div>
